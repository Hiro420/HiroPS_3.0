...
public class InvokeHandler<T> {
    ...
    public synchronized void addEntry(ForwardType forward, T entry) {
        switch (forward) {
            case FORWARD_TYPE_TO_ALL -> this.entryListForwardAll.add(entry);
            case FORWARD_TYPE_TO_ALL_EXCEPT_CUR, FORWARD_TYPE_TO_ALL_EXIST_EXCEPT_CUR ->
                this.entryListForwardAllExceptCur.add(entry);
            case FORWARD_TYPE_TO_HOST -> this.entryListForwardHost.add(entry);
            default -> {
            }
        }
    }
    ...
    public synchronized void update(Player player) {
        ...
        try {
            if (this.entryListForwardAll.size() > 0) {
                BasePacket packet = this.packetClass.getDeclaredConstructor(List.class).newInstance(this.entryListForwardAll);
                player.getScene().broadcastPacket(packet);
                this.entryListForwardAll.clear();
            }
            if (this.entryListForwardAllExceptCur.size() > 0) {
                BasePacket packet = this.packetClass.getDeclaredConstructor(List.class).newInstance(this.entryListForwardAllExceptCur);
                player.getScene().broadcastPacketToOthers(player, packet);
                this.entryListForwardAllExceptCur.clear();
            }
            if (this.entryListForwardHost.size() > 0) {
                BasePacket packet = this.packetClass.getDeclaredConstructor(List.class).newInstance(this.entryListForwardHost);
